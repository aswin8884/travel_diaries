...
import { MapPin, Search, Compass, Heart, Image as ImageIcon, ArrowRight } from 'lucide-react';
...
const Home = () => {
    ...
    useEffect(() => {
        const fetchDestinations = async () => {
            try {
                ...
                const res = await axios.get('http://localhost:8000/api/destinations/', { headers });
    ...
    const handleToggleLike = async (e, destId) => {
        e.stopPropagation();
        if (!isLoggedIn) { navigate('/login'); return; }
        if (likingId === destId) return;

        setLikingId(destId);
        try {
            const token = localStorage.getItem('access_token');
            const res = await axios.post(
                `http://localhost:8000/api/destinations/${destId}/toggle_like/`,
                {},
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setDestinations(prev =>
                prev.map(d => d.id === destId
                    ? { ...d, likes_count: res.data.likes_count, is_liked_by_me: res.data.is_liked_by_me }
                    : d
                )
            );
            // Keep selectedDest in sync so the modal reflects new like state
            if (selectedDest?.id === destId) {
                setSelectedDest(prev => ({ ...prev, likes_count: res.data.likes_count, is_liked_by_me: res.data.is_liked_by_me }));
            }
        } catch (error) { console.error(error); }
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-gray-950 font-sans">
            <div className="relative h-[calc(85vh+6rem)] -mt-24 flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <img
                        ...
                        alt="Hero background"
                    />
                    <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-black/20 to-white dark:to-gray-950"></div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-24">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-4">
                    <div className="max-w-xl">
                        <h2 className="text-4xl md:text-5xl font-black text-gray-900 dark:text-white tracking-tight mb-4">Trending Adventures</h2>
                        <p className="text-lg text-gray-500 dark:text-gray-400 font-medium">Curated destinations picked by our global community of travelers.</p>
                    </div>
                </div>
                    ...
                    {filteredDestinations.map((dest) => (
                        ...
                        >
                            <div className="relative h-[450px] rounded-[2.5rem] overflow-hidden mb-6 shadow-xl transition-all duration-500 group-hover:shadow-blue-500/20 group-hover:-translate-y-2">
                                ...
                                <button
                                    onClick={(e) => handleToggleLike(e, dest.id)}
                                    disabled={likingId === dest.id}
                                    title={isLoggedIn ? (dest.is_liked_by_me ? 'Unlike' : 'Like this destination') : 'Sign in to like'}
                                    className={`absolute top-5 right-5 flex items-center gap-1.5 px-3.5 py-2 rounded-2xl font-black text-sm shadow-lg backdrop-blur-md transition-all duration-200 active:scale-90
                                        ${dest.is_liked_by_me
                                ...
                                >
                                    <Heart
                                        size={16}
                                        fill={dest.is_liked_by_me ? 'currentColor' : 'none'}
                                        className={`transition-transform duration-200 ${likingId === dest.id ? 'scale-110' : ''}`}
                                    />
            ...
            <div className="bg-gray-50 dark:bg-gray-900 py-32 rounded-t-[5rem] border-t border-gray-100 dark:border-gray-800">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                        <div>
                            <div className="inline-flex items-center gap-2 text-blue-600 dark:text-blue-400 font-black text-sm uppercase tracking-widest mb-6">
                                <ImageIcon size={20} /> traveler feed
                            </div>
                            <h2 className="text-5xl md:text-6xl font-black text-gray-900 dark:text-white tracking-tighter mb-8 leading-tight">
                                Shared by our <span className="text-blue-600">Community.</span>
                            </h2>
                            <p className="text-xl text-gray-500 dark:text-gray-400 font-medium mb-10 leading-relaxed">
                            ...
                            <Link to="/community" className="inline-flex items-center gap-3 bg-gray-900 dark:bg-white dark:text-gray-900 text-white px-10 py-5 rounded-3xl font-bold text-lg hover:bg-blue-600 hover:text-white dark:hover:bg-blue-600 transition-all hover:gap-5 shadow-2xl">
