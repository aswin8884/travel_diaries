...
const RestaurantsTab = ({ destinations }) => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8000/api/restaurants/')
            .then(res => setRestaurants(res.data.results || res.data))
    ...
    if (restaurants.length === 0) return (
        <p className="text-center text-gray-500 dark:text-gray-400 py-12 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800">
            No dining spots found. Add one from the Command Center!
        </p>
    );
        ...
        <div className="animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
                {restaurants.map(rest => (
                    <div key={rest.id} className="bg-white dark:bg-gray-900 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 dark:border-gray-800 transition-all group relative hover:-translate-y-1">
                        ...
                        <div className="absolute top-4 right-4 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                            <button
                                ...
                                className="p-2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md text-gray-800 dark:text-gray-200 hover:bg-orange-500 hover:text-white rounded-lg shadow-sm transition-colors"
                            >
                                <PenTool size={16} />
                            </button>
                            <button
                        ...
                        <div className="p-5">
                            <div className="flex justify-between items-start mb-1">
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate pr-2">{rest.name}</h3>
                            ...
                            <p className="text-lg font-black text-gray-900 dark:text-white mt-2">
                                ₹{rest.average_cost} <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">avg cost</span>
                            </p>
                        </div>
                    </div>
                ))}
