...
import { UploadCloud, Building, MapPin, AlignLeft, Star, ArrowLeft, Save, Images } from 'lucide-react';
...
const AddHotel = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error(error);
            alert("Failed to add hotel. Ensure your backend Views handle 'gallery' uploads.");
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="min-h-screen bg-gray-50/30 pb-20 pt-10 px-6">
            <div className="max-w-4xl mx-auto relative z-10">
                <div className="bg-white/90 backdrop-blur-2xl p-10 rounded-[2.5rem] shadow-xl border border-gray-100 animate-in fade-in zoom-in-95 duration-500">
                    <Link to="/admin" className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-blue-600 mb-8 transition-colors bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100">
                        <ArrowLeft size={16} /> Back to Command Center
                    </Link>
                    
                    <div className="mb-10 pb-6 border-b border-gray-100">
                        <h2 className="text-3xl font-black text-gray-900 mb-2 flex items-center gap-3">
                            <Building className="text-blue-600" size={32}/> Add New Hotel
                        </h2>
                        <p className="text-gray-500 font-medium">Create a premium stay and link it to an existing destination.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 mb-8">
                            <label className="block text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                                <MapPin size={18} className="text-blue-500"/> Select Destination
                            </label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" step="1" required value={price} onChange={(e) => setPrice(e.target.value)} className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all font-medium text-lg" placeholder="12500" />
                            </div>
                            ...
                            <div className="p-5 bg-gray-50 rounded-2xl border border-gray-100">
                                <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2"><UploadCloud size={16}/> Main Cover Image</label>
                                <input type="file" required onChange={handleMainImageChange} accept="image/*" className="text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200 cursor-pointer w-full" />
                            ...
                            <div className="p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
                                ...
                                {gallery.length > 0 && <p className="text-xs text-blue-600 font-bold mt-2">{gallery.length} files selected.</p>}
                            </div>
                        </div>
                        ...
                        <div className="pt-8 border-t border-gray-100">
                            <button type="submit" disabled={isSaving} className="w-full py-5 bg-gray-900 hover:bg-black text-white font-bold rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all flex items-center justify-center gap-2 text-xl">
                                {isSaving ? 'Saving to Database...' : <><Save size={24} /> Publish Hotel</>}
                            </button>
                        </div>
                    </form>
                </div>
